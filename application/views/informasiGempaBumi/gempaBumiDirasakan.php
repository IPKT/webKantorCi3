<div class="card">
    <div class="card-header" style="background-color:#343f52; color: white;">
        <div>
            <h5>Gempa Bumi Dirasakan Wilayah Bali</h5>
        </div>
    </div>
    <div class="card-body">
        <div class="container">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped small">
                    ...
                    <tbody>
                        <?php 
        $this->load->database();
        $query = $this->db->query('SELECT infogempa FROM gempabalidirasakan ORDER BY id DESC LIMIT 20');
        ...
        foreach ($results as $row):
            list($waktu,$lintang,$bujur,$magnitude,$kedalaman,$lokasi) = prosesHTML($row->infogempa,"no");?>
                        <tr>
                            ...
                            <td style=" white-space: nowrap;"><?php echo $lintang?> , <?php echo $bujur?></td>
...
$this->load->database();
$query = $this->db->query('SELECT infogempa FROM gempabalidirasakan ORDER BY id DESC LIMIT 100');
$results   = $query->result();
$no = 1;
$warna = "";
foreach ($results as $row):
...
function prosesHTML($event, $cekDB){
    ...
    if ($cekDB == "yes") {
        // $db = \Config\Database::connect();
        $this->load->database();
        $query = $this->db->query("SELECT infogempa FROM gempabali WHERE id='$id'");
        $row   = $query->getRow();
        if ($row != NULL) {
